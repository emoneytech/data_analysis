class DayCustomersWorker
  ...
  def perform()
    ...
    CustomerEvaluation.select(:anagrafica_id).where(eval_year: Date.today.year, eval_month: Date.today.month).where.not('eval_days ?| array[:keys]', keys: ["#{Date.today}"]).find_in_batches(batch_size: 500) do |evaluations|
