class CreateEvaluatedMovementWorker
  ...
  def perform(service_id, point, default_product_base_risk = Configurable.default_product_base_risk.to_f)
    ...
    if internal_beneficiary = service.try(:bonifico).try(:internal_beneficiary)
      em2 = EvaluatedMovement.where(triggerable_type: "Servizio", triggerable_id: service.id, customer_id: internal_beneficiary.id).first_or_initialize
      em2.payer = internal_beneficiary.full_name

      em2.triggerable_status =  em.service_status
