...
class EvaluatedMovement < CorePgRecord
  ...
  validates :triggerable_id,
            uniqueness: {
              scope: %i[customer_id triggerable_type],
            }
  ...
  def previous
    EvaluatedMovement
      .where(
        'customer_id = ? AND movement_created_at < ? ',
        self.customer_id,
        self.movement_created_at,
      )
  ...
  def build_from_movement
    ...
    movement =
      begin
        Movimentoconto.only_customers.find movement_id
  ...
  def set_beneficiary_from_mandato(mandato = self.mandato)
    ...
    if destination_account
      unless destination_account.current_place
        destination_account.set_current_place
      end
  ...
  def set_beneficiary_from_service(service = self.service)
    ...
    when 'ricarichecarta'
      if service
           .anagrafica
           .conti
           .where(Pan: service.ricaricacarta.numerocarta)
           .count > 0
        ...
        unless destination_account
          ...
          self.destination_lonlat =
            destination_account.reload.current_place.lonlat
          self.internal = true
        end
      end
      self.destination_country = 'MT'
    ...
    when 'assegnovirtuale'
      if service.prodotto.to_s == '1614' || service.prodotto.to_s == '1612'
        movement =
          service
            .movimenticonti
            .where(Point: service.anagrafica.id)
            .where(contodiprovenienza: service.anagrafica.conti.pluck(:Pan))
            .first
        self.beneficiary = "#{movement.Causale}"
      elsif service.prodotto.to_s == '200618' ||
            service.prodotto.to_s == '200619' ||
            service.prodotto.to_s == '200620'
        self.beneficiary = 'Beneficiary not identifiable'
      else
  ...
  def set_destination_lonlat
    if self.beneficiary_iban && self.beneficiary_iban != ''
      ...
      self.destination_lonlat =
        "POINT(#{iban_info['position']['longitude']} #{iban_info['position']['latitude']})"
    end
  end
  ...
  def recursive_for_customer(days = 7)
    ...
    return(
      ...
      OR (evaluated_movements.beneficiary_card != '' AND evaluated_movements.beneficiary_card = ?))",
          self.beneficiary.gsub("\\", '-'),
          self.beneficiary_iban,
          self.beneficiary_card,
        )
        .where(
          "(evaluated_movements.payer ilike ? 
      OR (evaluated_movements.payer_iban != '' AND evaluated_movements.payer_iban = ?)
      OR (evaluated_movements.payer_card != '' AND evaluated_movements.payer_card = ?))",
          self.payer.gsub("\\", '-'),
          self.payer_iban,
          self.payer_card,
        )
  ...
  def countries
    [
      RelatedCountry.find_by_alpha2(self.origin_country).try(:id),
      RelatedCountry.find_by_alpha2(self.destination_country).try(:id),
    ]
  end

  def observers
    ObservedElement
      .filter_by_iban(self.ibans)
      .or(ObservedElement.filter_by_customer_id(self.customer_id))
      .or(ObservedElement.filter_by_user(self.beneficiary))
      .or(ObservedElement.filter_by_user(self.payer))
  end

  def send_enhanced_notification
    self.observers.each do |observer|
      EvaluatedMovementNotification
        .with(evaluated_movement: self, observer: observer)
        .deliver_later(User.recipients)
    end
    ObservedElement.common_processes.each do |observer|
      if observer.category_element == 'sanction_list'
        count =
          SanctionListItem.name_similar(self.payer).count +
            SanctionListItem.name_similar(self.beneficiary).count
        if count > 0
          EvaluatedMovementNotification
            .with(evaluated_movement: self, observer: observer)
            .deliver_later(User.recipients)
