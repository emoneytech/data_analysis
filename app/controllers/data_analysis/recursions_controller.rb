module DataAnalysis
  class RecursionsController < ApplicationController
    ...
    def chart
      ...
      recursions = Hash.new
      EvaluatedMovement.where.not(field => nil, field => 0).for_month(Date.today - 1.month).group(field).group_by_day(:movement_created_at).order(field => :asc).count.map{|k,v| recursions[ [k[0],k[1]] ] = v}
